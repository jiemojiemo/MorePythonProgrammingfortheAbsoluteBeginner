...
def draw_circle(x, y, radius, width = 0):
    pygame.draw.circle(screen, black, (int(x)-4,int(y)-4), radius, width)
    pygame.draw.circle(screen, yellow, (int(x),int(y)), radius, width)


def draw_bomb(x, y):
    draw_circle(x, y, 30)
    # draw lead wire on bomb
    leadWireColor = [random.randint(0,255) for _ in range(3)]
    position = x, y-30-50, 100, 100
    start_angle = math.radians(90)
    end_angle = math.radians(180)
    pygame.draw.arc(screen, leadWireColor, position, start_angle, end_angle,2)
...
# pause the game if failed to catch the bomb
failedCatch = False
clock_start = 0
seconds = 3
    ...
    if gameOver:
        ...
        if bomb_y > 500:
            ...
            if bomb_x > pos_x and bomb_x < pos_x + 120:
                ...
                bomb_y = -50
                vel_x = random.choice([1,-1])*vel_x
        # collide agaist to the left side or right side
        if bomb_x < 30 or bomb_x > 570:
            vel_x = -vel_x
